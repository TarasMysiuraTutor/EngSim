...
- Виправлено: Footer не відображався на жодній сторінці.
  - Причина: при видаленні дублів Navbar/Footer зі сторінок (v1.5.0) Footer не було додано в `AppRoutes.jsx`.
  - `AppRoutes.jsx` — додано `<Footer>` після `</Suspense>`.
  - `AdminPage.jsx` — прибрано власний `<Footer>` (тепер рендериться з AppRoutes).

### v1.9.0 — 2026-05-21

- **Повне очищення коду** — весь `src/` перевірено і відчищено:
  - Прибрано всі зайві ` `` ;` рядки: `AdminPage.jsx`, `ProjectsTab.jsx`, `water.properties.js`.
  - Всі emoji-коментарі (`// ✅`, `// ❌`, `// ⚠️`) замінено на звичайний текст або прибрано: 10+ файлів у `admin/`, `data/`, `utils/`, `pages/`.
  - `pdfExport.jsx` — прибрано закоментовані import рядки і emoji.
- **Виправлення**:
  - `AcademicProfiles.jsx` — виправлено `<picture>` тег для WebP (попередній патч не спрацював коректно).
- **Результат**: `npm run build` проходить чисто, у всьому `src/` нуль ` `` ;` і emoji-коментарів.

### v1.8.0 — 2026-05-21

- **Критичне виправлення** — `VideosPage.jsx` більше не падає:
  - Змінна `breadcrumbs` використовувалась без визначення → ReferenceError.
  - Зайвий ` `` ;` після `buildBreadcrumbJsonLd` → SyntaxError у runtime.
  - Прибрано невикористаний імпорт `Helmet` і закоментований блок `<Helmet>`.
- **Виправлено аналогічні проблеми** у ще 5 сторінках: `ContactPage`, `ServicesPage`, `LibraryPage`, `ProjectsPage`, `AboutPage` — всі мали зайвий ` `` ;` рядок.
- **Чистота коду**: прибрано emoji-коментарі (`// ✅`, `// ❌`) з `TestimonialsPage` і `FAQPage`.

### v1.7.0 — 2026-05-20

- **Service Worker** — повністю переписано `public/service-worker.js`:
  - Виправлено критичний баг: `BASE_PATH = "/EngSim"` замінено на `"/"` (старий GitHub Pages шлях блокував кешування на Vercel).
  - Прибрано `console.log` з SW коду.
  - Нова стратегія кешування: Network First для HTML (SSG контент завжди актуальний), Cache First для JS/CSS/зображень.
  - `CACHE_NAME` оновлено з `engsim-v1` → `engsim-v2` (очищає старий хибний кеш у користувачів).
- **Оптимізація зображень**:
  - `src/assets/dspace-logo.png` → додано WebP версію (13KB → 5KB, −53%).
  - `AcademicProfiles.jsx` — використовує `<picture>` з WebP і PNG fallback.
