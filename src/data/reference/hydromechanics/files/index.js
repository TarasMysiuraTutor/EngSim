...
// src/data/reference/hydromechanics/files/index.js
import fileRegistry from "./fileRegistry.json";

// Допоміжні функції
export const getFilesByCategory = (category) => {
  return {
    documents: fileRegistry.documents.filter((f) => f.category === category),
    diagrams: fileRegistry.diagrams.filter((f) => f.category === category),
  };
};

export const getFeaturedFiles = () => {
  return fileRegistry.documents.filter((f) => f.featured);
};

export const searchFiles = (query, lang = "uk") => {
  const lowerQuery = query.toLowerCase();
  return fileRegistry.documents.filter(
    (file) =>
      file.title[lang]?.toLowerCase().includes(lowerQuery) ||
      file.description?.[lang]?.toLowerCase().includes(lowerQuery) ||
...
export const getFileUrl = (file) => {
  ...
  if (file.storage === "github-release") {
    return file.path; // вже повний URL
  }
  if (file.storage === "cdn") {
    return `https://cdn.yourdomain.com${file.path}`;
  }
  return file.path; // локальний шлях
};

export const hydromechanicsFiles = {
  documents: fileRegistry.documents,
  diagrams: fileRegistry.diagrams,
  videos: fileRegistry.videos || [],
  categories: fileRegistry.categories,

  // Методи
  getByCategory: getFilesByCategory,
  getFeatured: getFeaturedFiles,
  search: searchFiles,
  getUrl: getFileUrl,
};
